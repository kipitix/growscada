...
type TagService interface {
	FindAllTags(context.Context) ([]app_dto.Tag, error)
	FindTagByID(context.Context, tag.TagID) (app_dto.Tag, error)
	CreateTag(context.Context, app_dto.CreateTagInput) (app_dto.Tag, error)
	DeleteTagByID(context.Context, tag.TagID) (app_dto.Tag, error)
	SetTagValueByID(context.Context, app_dto.UpdateTagInput) (app_dto.Tag, error)
}
...
func (t tagServiceImpl) FindAllTags(ctx context.Context) ([]app_dto.Tag, error) {
	tags, err := t.tagRepository.FindAll(ctx)
	if err != nil {
		return nil, fmt.Errorf("error on find tags in repository: %w", err)
	}

	return app_dto.NewTagList(tags), nil
}

// FindTagByID returns a tag by its identifier
func (t tagServiceImpl) FindTagByID(ctx context.Context, tagID tag.TagID) (app_dto.Tag, error) {
	foundTag, err := t.tagRepository.FindByID(ctx, tagID)
	if err != nil {
		return app_dto.Tag{}, fmt.Errorf("error on find tag by id in repository: %w", err)
	}

	return app_dto.NewTag(foundTag), nil
}

// CreateTag creates a new tag and returns the created tag DTO
func (t tagServiceImpl) CreateTag(ctx context.Context, newTagData app_dto.CreateTagInput) (app_dto.Tag, error) {
	newTagID := t.tagRepository.NextID()

	newTagName, err := tag.NewTagName(newTagData.Name)
	if err != nil {
		return app_dto.Tag{}, fmt.Errorf("cannot create tag because of name: %w", err)
	}

	newTagType, err := tag.NewTagType(newTagData.Type)
	if err != nil {
		return app_dto.Tag{}, fmt.Errorf("cannot create tag because of type: %w", err)
	}

	newTagValue, err := newTagType.NewTagValue(newTagData.Value)
	if err != nil {
		return app_dto.Tag{}, fmt.Errorf("cannot create tag because of value: %w", err)
	}

	newTagQuality, err := tag.NewTagQuality(newTagData.Quality)
	if err != nil {
		return app_dto.Tag{}, fmt.Errorf("cannot create tag because of quality: %w", err)
	}

	newTag, err := tag.NewTag(newTagID, newTagName, newTagType, newTagValue, newTagQuality, tag.TagVersionInitial)
	if err != nil {
		return app_dto.Tag{}, fmt.Errorf("cannot create tag: %w", err)
	}

	err = t.tagRepository.Save(ctx, newTag)
	if err != nil {
		return app_dto.Tag{}, fmt.Errorf("cannot save tag: %w", err)
	}

	t.eventBus.Publish(event.NewTagCreatedEvent(newTagID))

	return app_dto.NewTag(newTag), nil
}

// DeleteTagByID deletes a tag by its identifier and returns the deleted tag
func (t tagServiceImpl) DeleteTagByID(ctx context.Context, tagIDToDelete tag.TagID) (app_dto.Tag, error) {
	deletedTag, err := t.tagRepository.DeleteByID(ctx, tagIDToDelete)
	if err != nil {
		return app_dto.Tag{}, fmt.Errorf("cannot delete tag: %w", err)
	}

	t.eventBus.Publish(event.NewTagDeletedEvent(tagIDToDelete))

	return app_dto.NewTag(deletedTag), nil
}

// SetTagValueByID updates the value and quality of an existing tag and returns the updated tag
func (t tagServiceImpl) SetTagValueByID(ctx context.Context, request app_dto.UpdateTagInput) (app_dto.Tag, error) {
	...
	if err != nil {
		return app_dto.Tag{}, fmt.Errorf("cannot parse quality: %w", err)
	}

	if err = foundTag.SetValue(request.Value, newQuality); err != nil {
		return app_dto.Tag{}, fmt.Errorf("cannot set tag value: %w", err)
	}
